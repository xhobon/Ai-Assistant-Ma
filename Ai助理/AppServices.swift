...
struct CloudConversationSummary: Codable, Identifiable {
    let id: String
    let title: String
...
final class APIClient {
    ...
    /// 获取当前账号会话列表（用于换设备后同步最近对话）
    func getConversations(take: Int = 20) async throws -> [CloudConversationSummary] {
        struct Res: Codable { let conversations: [CloudConversationSummary] }
        let safeTake = max(1, min(take, 100))
        let res: Res = try await request("api/conversations?take=\(safeTake)", method: "GET", authorized: true)
        return res.conversations
    }

    /// 获取某个会话的全部消息
    func getConversationMessages(conversationId: String) async throws -> [ChatMessage] {
        struct Res: Codable { let messages: [CloudConversationMessageDTO] }
        let escapedId = conversationId.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? conversationId
        let res: Res = try await request("api/conversations/\(escapedId)/messages", method: "GET", authorized: true)
        let formatter = ISO8601DateFormatter()
        return res.messages.compactMap { item in
            guard let role = ChatRole(rawValue: item.role) else { return nil }
    ...
    func getMemories() async throws -> [UserMemoryItem] {
        ...
        return res.memories.map { m in
            UserMemoryItem(
                ...
                expiresAt: m.expiresAt.flatMap { formatter.date(from: $0) },
                source: m.source,
    ...
    /// 上传记忆到云端（合并，去重）
    func addMemories(_ items: [(content: String, category: String, confidence: Double?, ttlDays: Int?, source: String?)]) async throws {
        struct Res: Codable { let success: Bool }
        let payload: [[String: Any]] = items.map { item in
            var row: [String: Any] = [
                "content": String(item.content.prefix(500)),
                "category": item.category
            ]
            if let c = item.confidence { row["confidence"] = c }
            if let ttl = item.ttlDays { row["ttlDays"] = ttl }
...
final class SpeechService: NSObject, ObservableObject {
    ...
    /// 是否应过滤识别到的文本（疑似自身播报回采）
    func shouldFilterRecognizedText(_ text: String) -> Bool {
        let normalized = normalizeForEchoCheck(text)
        guard !normalized.isEmpty, !playbackGuardText.isEmpty else { return false }
        // 仅在播放中或播放后短时间内启用
        if !isPlaying && Date() > playbackGuardUntil { return false }
        if playbackGuardText.contains(normalized) { return true }
        let guardPrefix = String(playbackGuardText.prefix(min(16, playbackGuardText.count)))
        if normalized.count >= 8, !guardPrefix.isEmpty, normalized.contains(guardPrefix) { return true }
        return false
    }
    
    private func beginPlaybackGuard(with text: String) {
        let normalized = normalizeForEchoCheck(text)
