...
- **iOS 应用里配置的地址**: `https://ai-assistant-mac.vercel.app`  
...
### 5. OpenClaw 完整模式（内置 Gateway / 打开即用）

- **使用内置 Gateway（打开即用）**：应用内打包 Node.js + OpenClaw，无需本机安装。  
  - 首次打包需在项目根目录运行：`./scripts/prepare-openclaw-bundle.sh`  
  - 首次使用时在 **本机执行** 设置页点击「配置 OpenAI API Key」完成 onboarding  
  - 开启「使用 OpenClaw 完整模式」+「使用内置 Gateway」即可使用

- **连接本机已运行的 Gateway**：若已通过 `npm i -g openclaw` 安装，可选择「连接本机已运行的 Gateway」，并填写 `ws://127.0.0.1:18789` 等地址。

### 6. 本机执行（内置命令执行）

- 后端对话接口支持 **`localExecution: true`**：助理可返回 `[CMD]命令[/CMD]`，由 iOS 应用在用户确认后执行（仅首页入口、白名单命令、结果发送前二次确认，保护隐私）。
- **需要重新部署后端** 才能生效；iOS 端需重新编译。

#### 为什么本机执行无法使用？排查清单

1. **未在应用内打开开关**  
   - 打开 **个人中心 → 本机执行**，确认 **「在助理中使用本机执行」** 已开启。

2. **对话入口不支持本机执行**  
   - 只有从 **首页主入口**（如「智能对话」「语音助手」「图像识别」或「AI智能助理」卡片）进入的助理对话才会带本机执行。
   - 从 **法律顾问、投资顾问、面试官、AI编程、营销策划** 等单独服务卡片进入的对话，当前不会启用本机执行。

3. **后端未部署或未更新**  
   - 若 Vercel 上的 `server` 未部署包含 `localExecution` 的代码，接口不会给 AI 注入「可输出 [CMD]…[/CMD]」的说明，AI 就不会返回命令。
   - 解决：在 `server` 目录推送最新代码并触发 Vercel 重新部署（见下文「完整部署流程」）。

4. **iOS 应用请求的地址不是最新后端**  
   - 应用内写死的地址为 `https://ai-assistant-mac.vercel.app`。若该域名对应的 Vercel 项目不是当前 `server` 仓库或未重新部署，请求会打到旧接口。
   - 解决：在 Vercel 中确认对应项目的 Root Directory 为 `server` 并重新部署；或按「方案 B」把应用改为你已部署的地址（如 `https://traefmhs2e5i.vercel.app`）后重新编译。

5. **对话内容未触发命令**  
   - 本机执行仅在 AI 认为需要「查看本机信息」时才会返回 `[CMD]...[/CMD]`（如列出目录、读文件、系统信息等）。
   - 可尝试明确提问，例如：「列出我当前用户主目录下的文件」或「当前目录是哪里？」以触发 `ls` / `pwd` 等安全命令。

6. **当前为「带文件上传」的对话**  
   - 上传文件后发送消息走的是「带文件」的接口，该接口目前**未传** `localExecution`，因此带文件对话时本机执行不会生效；仅普通文字/图片消息对话会带本机执行。
...
- **改动了 iOS 应用 Swift 代码**（例如：新功能、本机执行开关、修复崩溃等）→ 需要 **重新编译/打包 iOS 应用**。
...
1. 用 Xcode 打开项目：
   - 打开 `Ai助理.xcodeproj` 或 `Ai助理.xcworkspace`（若用 CocoaPods）。
2. 选择 iOS 目标（手机图标的 target）。
3. 运行 / 打包：
...
- [ ] iOS 应用：已在 Xcode 中重新 Run 或 Archive，确保安装的是最新构建。
